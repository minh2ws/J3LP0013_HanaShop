...
public class UpdateStatusProductServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            dao.updateStatusQuestion(productId, status);
            
            HttpSession session = request.getSession(false);
            TblUserDTO user = (TblUserDTO) session.getAttribute("ACCOUNT");
            TblLogDAO log = new TblLogDAO();
            String logId = GenerateCode.generateLogID();
            log.writeLog(logId, productId, user.getUserID());
        } catch (SQLException e) {
            log("UpdateStatusProductServlet _ SQLException: " + e.getMessage());
        } catch (NamingException e) {
            log("UpdateStatusProductServlet _ NamingException: " + e.getMessage());
        }  finally {
